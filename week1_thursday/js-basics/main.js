...
let formatDate = true;
const changeDateFormatBtn = document.getElementById('changeDateFormatButton');

// Visa dagens datum
updateDate();

// Byt datumformat vid knapptryck https://www.w3schools.com/js/js_htmldom_eventlistener.asp
changeDateFormatBtn.addEventListener('click', function () {
...
function updateDate() {
  ...
  if (formatDate) {
    currentDate.textContent = "Här visas dagens datum: " + dagensDatum.toLocaleDateString('sv-SE');
  } else {
    currentDate.textContent = "Här visas dagens datum: " + dagensDatum.toDateString();
  }
}
